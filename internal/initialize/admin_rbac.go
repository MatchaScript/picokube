...
// this whole refactor.
// lae is dereferenced unconditionally by the helper (it rewrites the
// loaded kubeconfig's server to the local endpoint), so it must not be
// nil; upstream cmd/kubeadm/app/cmd/init.go passes &cfg.LocalAPIEndpoint
// the same way.
func initAdminRBAC(l layout.Layout, lae *kubeadmapi.APIEndpoint) (kubernetes.Interface, error) {
	client, err := kubeconfig.EnsureAdminClusterRoleBinding(l.KubernetesDir, lae, nil)
	if err != nil {
